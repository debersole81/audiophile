        ...
        <Col className="col mt-1" xs={12} s={12} md={5} lg={6} xl={7}>
          ...
          <Row className="album-details">
            <Col className="col" xs={8} s={8} md={8} lg={7} xl={7}>
              {albumData.formats ? (
                <p>
                  <strong>Format:</strong> {albumData.formats[0].name}{" "}
                </p>
              ) : null}
              {albumData.genres ? (
                <p>
                  <strong>Genre:</strong> {albumData.genres[0]}
                </p>
              ) : null}
              {albumData.styles ? (
                <p>
                  <strong>Style:</strong> {albumData.styles.join(", ")}
                </p>
              ) : null}
              {albumData.labels ? (
                <p>
                  <strong>Label:</strong> {albumData.labels[0].name}
                </p>
              ) : null}
              {albumData.labels ? (
                <p>
                  <strong>Cat #:</strong> {albumData.labels[0].catno}
                </p>
              ) : null}
              {albumData.country ? (
                <p>
                  <strong>Release Country:</strong> {albumData.country}
                </p>
              ) : null}
              {albumData.year ? (
                <p>
                  <strong>Release Year:</strong> {albumData.year}
                </p>
              ) : null}
            </Col>
            {albumData.id === albumMasterData.main_release ? (
              <Col
                className="col album-master-logo-col"
                xs={4}
                s={4}
                md={4}
                lg={5}
          ...
          <Row className="album-add-buttons-row">
            <Col>
              {userCollectionAlbums.some(
                (element) => element.albumId === albumData.id
              ) ? (
                <Button
                  variant="dark"
                  size="sm"
                  id={collectionAlbumId}
                  onClick={deleteAlbumFromCollection}
                  block
                >
