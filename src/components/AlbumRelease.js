...
function AlbumRelease(props) {
  ...
  return (
    <Container className="mt-3">
      <Row>
        <Col
          className="col album-release-image-col"
          xs={12}
          s={12}
          md={7}
          lg={6}
        ...
        >
          <Image
            fluid
            src={albumReleaseData.images[0].uri}
            alt="Album Cover Art"
          />
          {Array.isArray(images) && images.length ? (
            <Button
              variant="dark"
              className="album-release-images-button"
        ...
        <Col className="col mt-1" xs={12} s={12} md={5} lg={6} xl={7}>
          ...
          <Row className="album-release-details">
            <Col>
              {albumReleaseData.formats ? (
                <p>
                  <strong>Format:</strong> {albumReleaseData.formats[0].name}{" "}
                </p>
              ) : null}
              {albumReleaseData.genres ? (
                <p>
                  <strong>Genre:</strong> {albumReleaseData.genres[0]}
                </p>
              ) : null}
              {albumReleaseData.styles ? (
                <p>
                  <strong>Style:</strong> {albumReleaseData.styles.join(", ")}
                </p>
              ) : null}
              {albumReleaseData.labels ? (
                <p>
                  <strong>Label:</strong> {albumReleaseData.labels[0].name}
                </p>
              ) : null}
              {albumReleaseData.labels ? (
                <p>
                  <strong>Cat #:</strong> {albumReleaseData.labels[0].catno}
                </p>
              ) : null}
              {albumReleaseData.country ? (
                <p>
                  <strong>Release Country:</strong> {albumReleaseData.country}
                </p>
              ) : null}
              {albumReleaseData.year ? (
                <p>
                  <strong>Release Year:</strong> {albumReleaseData.year}
                </p>
              ) : null}
            </Col>
          </Row>
          <Row className="album-release-add-buttons-row">
            <Col>
              {userCollectionReleases.some(
                (element) => element.albumId === albumReleaseData.id
              ) ? (
                <Button
                  variant="dark"
                  size="sm"
                  id={collectionReleaseId}
                  onClick={deleteReleaseFromCollection}
                  block
                >
                  <FaRecordVinyl /> Remove From Collection
                </Button>
              ) : (
                <Button
                  variant="dark"
                  size="sm"
                  onClick={addReleaseToCollection}
                  block
                >
                  <FaRecordVinyl /> Add To Collection
                </Button>
              )}
              {userWishListReleases.some(
                (element) => element.albumId === albumReleaseData.id
              ) ? (
                <Button
                  variant="dark"
                  size="sm"
                  id={wishListReleaseId}
